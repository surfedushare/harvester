...
ENDPOINT = FontysPureResource.URI_TEMPLATE.replace("https://", "")


class FontysPureResourceFactory(factory.django.DjangoModelFactory):

    class Meta:
        model = FontysPureResource
    ...
    def uri(self):
        offset = self.number * 5  # 5 items per page in our test fixtures
        return f"{ENDPOINT}?offset={offset}&size=5" if offset else ENDPOINT
    ...
    def body(self):
        ...
        response_file = f"fixture.fontys.{response_type}.{self.number}.json"
