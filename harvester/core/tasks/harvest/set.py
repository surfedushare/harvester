...
def dispatch_set_task_retry(task, exc, task_id, args, kwargs, einfo):
    ...
    logger = HarvestLogger(app_label, "dispatch_set_task_retry", {}, is_legacy_logger=False)
    harvest_set = load_pending_harvest_instances(harvest_set_id, model=storages.Set)
    ...
    logger.report_cancelled_documents(app_label, harvest_set.name, pending_document_count)
...
def dispatch_set_tasks(app_label: str, harvest_set: int | HarvestSet, asynchronous: bool = True,
    ...
    storages = load_harvest_models(app_label)
    harvest_set = load_pending_harvest_instances(harvest_set, model=storages.Set)
    if harvest_set is None:  # parallel tasks may already picked-up this dispatch
        return
    pending = validate_pending_harvest_instances(harvest_set, model=storages.Set)
    if len(pending):
    ...
    elif validate_pending_harvest_instances(harvest_set.dataset_version, model=storages.DatasetVersion):
