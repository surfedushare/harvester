...
@atomic()
def lookup_organization_parents(app_label: str, set_id: int):
    storages = load_harvest_models(app_label)
    Set = storages.Set
    Document = storages.Document

    # Get the set and all its documents in a single query
    organization_set = Set.objects.select_for_update().get(id=set_id)
